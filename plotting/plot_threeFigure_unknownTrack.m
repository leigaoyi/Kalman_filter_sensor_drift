...
% 
distance_measure = mean(abs(input_curve-track_input))
% 
...
subplot(2, 1, 1); % Select the second subplot
...
distance_unknown = mean(input_curve - track_input)
distance_known = mean(input_curve - track_knwon_input)
...
% Adjust the position of the legend
newLegendPosition = lgd.Position + [-0.02, -0.027, 0, 0.00]; % Move slightly to the left
...
a4 = plot(time_axis, y_unknown_curve, 'k', 'LineWidth', 1.5);
hold off;
...
legend('KF error std on known signal', 'KF error std on partially-known signal', 'Measurement error std', 'Location', 'northeast','box','off','FontSize', 12);
...
text(-0.07, 1.00, '(b)', 'Units', 'normalized', 'FontSize', 10, 'FontWeight', 'bold', 'HorizontalAlignment', 'right');
