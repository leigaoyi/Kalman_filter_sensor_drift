...
measure_std = abs(measure_std/kresponse);
track_std = abs(track_std/kresponse);
...
ylabel('Measurement & KF Uncertainty  (pT)', 'FontSize', 12);
legend('Measurement std', 'KF estimate std', 'FontSize', 10, 'Box', 'off');
grid on;
box on;
xlim([0.5, 50.5]);
...
print('../result/Measurement_KF_std', '-dpng', '-r600'); % Specify the desired image format and resolution
